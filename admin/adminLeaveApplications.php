...
if(isset($_GET['type']) && $_GET['type']!=''){
	...
	if($type=='status'){
		...
		$update_status_sql="update leaveapplications set status='$status' where id='$id'";
		mysqli_query($con,$update_status_sql);
	}
	
	if($type=='delete'){
		$LAid=get_safe_value($con,$_GET['id']);
		$delete_sql="delete from leaveapplications where LAid='$id'";
			...
			<!-- Striped table start -->
				...
				<div class="pd-20 card-box mb-30">
					<div class="clearfix mb-20">
						<div class="pull-left">
							<h4 class="text-blue h4">OPERATING DETAIL</h4>
							  <button type="button" class="btn btn-outline-info"> <h6 class="box-link"><a href="manage_operation.php?type=add&machine_id=<?=$_GET['id']?>">ADD MACHINE OPERATION</a> </h6>  </button>
						</div>
					</div>
						...
						<tbody>
							...
							<td><?php echo $row['id']?></td>
							   ...
							   <td><?php echo $row['numberofdays']?></td>
								...
								if($row['status']==0)
								{
									echo "<a href='?type=status&operation=deactive&id=".$row['id']."'><h6 style=color:blue;>Pending</h6></a>&nbsp;";
								}
								elseif($row['status']==1)
								{
									echo "<a href='?type=status&operation=deactive&id=".$row['id']."'><h6 style=color:green;>Approved</h6></a>";
								}
								else
								{
									echo "<a href='?type=status&operation=active&id=".$row['id']."'><h6 style=color:red;>Rejected</h6></a>";
								}
								?></td>

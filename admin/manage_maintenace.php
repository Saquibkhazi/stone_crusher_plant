...
if(isset($_GET['id']) && $_GET['id']!=''){
	...
	$res=mysqli_query($con,"select * from machinemaintenances where id='$id'");
	...
	{
		header('location:maintenace.php?id='.$id);
		die();
	}
}
if(isset($_POST['submit'])){
	$date=get_safe_value($con,$_POST['date']);
	$duedate=get_safe_value($con,$_POST['duedate']);
	$remarks=get_safe_value($con,$_POST['remarks']);
	$machine_id=get_safe_value($con,$_POST['machine_id']);
	$employee_id=get_safe_value($con,$_POST['employee_id']);
/*	$res=mysqli_query($con,"select * from machinemaintenances where machine_id='$machine_id'");
	$check=mysqli_num_rows($res);
	if($check>0){
		if(isset($_GET['id']) && $_GET['id']!=''){
			...
			if($id==$getData['id']){
				...
				$msg="machinemaintenances already exist";
			}
		}else{
			$msg="machinemaintenances already exist";
		}
	...
	if(true){
		if(isset($_GET['id']) && $_GET['id']!=''){
			{
				$update_sql="update machinemaintenances set date='$date',duedate='$duedate',remarks='$remarks',machine_id='$machine_id',employee_id='$employee_id' where id='$id'";
			}
			mysqli_query($con,$update_sql);
		}
		else
		{
			mysqli_query($con,"insert into machinemaintenances (date,duedate,remarks,machine_id,employee_id)values('$date','$duedate','$remarks','$machine_id','$employee_id')");
		}
		header("location:maintenace.php?id=".$machine_id);
		die();
	}
}
 $sql="select * from machinemaintenances where machine_id='$id'";
...
<body>
	...
	<div class="form-group row">
		<label for="name" class="col-sm-12 col-form-label"><div class="weight-600 font-30 text-blue">MAINTENANCE</div></label>			
	</div>			
	<div class="form-group row">
		<label for="name" class="col-sm-12 col-form-label"><h6>DATE </h6></label>
		<div class="col-sm-12 col-md-10">
			<input type="date" name="date" placeholder="ENTER DATE" class="form-control" required value="<?php echo $date?>">
		</div>
	</div>
	<div class="form-group row">
		<label for="name" class="col-sm-12 col-form-label"><h6>DUE DATE</h6></label>
		<div class="col-sm-12 col-md-10">
		<input type="date" name="duedate" placeholder="ENTER DUE DATE" class="form-control" required value="<?php echo $duedate?>">
		</div>
	</div>
	<div class="form-group row">
		<label for="name" class="col-sm-12 col-form-label"><h6>REMARK</h6></label>
		<div class="col-sm-12 col-md-10">
		<input type="text" name="remarks" placeholder="ENTER REMARK" class="form-control" required value="<?php echo $remarks?>">
		</div>
	</div>
	<div class="form-group row">
		<label for="name" class="col-sm-12 col-form-label"><h6>MACHINE</h6></label>
		<div class="col-sm-12 col-md-10">
		<select class="form-control" name="machine_id">
										<option>SELECT MACHINE</option>
										<?php
										$res=mysqli_query($con,"select id,name from machines");
	...
	<div class="form-group row">
		<label for="name" class="col-sm-12 col-form-label"><h6>MAINTENANCE EMPLOYEE</h6></label>
		<div class="col-sm-12 col-md-10">
		<select class="form-control" name="employee_id">
										<option>SELECT  MAINTENANCE EMPLOYEE</option>
										<?php
										$res=mysqli_query($con,"select id,name from employees where department_id=4");
										while($row=mysqli_fetch_assoc($res)){
											if($row['id']==$company_id){
												echo "<option selected value=".$row['id'].">".$row['name']."</option>";
											}else{

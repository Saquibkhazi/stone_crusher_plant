...
if(isset($_GET['id']) && $_GET['id']!=''){
	...
	$res=mysqli_query($con,"select * from operations where id='$id'");
	...
	{
		header('location:operation.php?id='.$id);
		die();
	}
}
if(isset($_POST['submit'])){
	$date=get_safe_value($con,$_POST['date']);
	$starttime=get_safe_value($con,$_POST['starttime']);
	$endtime=get_safe_value($con,$_POST['endtime']);
	$machine_id=get_safe_value($con,$_POST['machine_id']);
	$employee_id=get_safe_value($con,$_POST['employee_id']);
/*	$res=mysqli_query($con,"select * from operations where name='$name'");
	$check=mysqli_num_rows($res);
	if($check>0){
		if(isset($_GET['id']) && $_GET['id']!=''){
...
*/
	if(true){
		if(isset($_GET['id']) && $_GET['id']!=''){
			{
				$update_sql="update operations set date='$date',starttime='$starttime',endtime='$endtime',machine_id='$machine_id',employee_id='$employee_id' where id='$id'";
			}
			mysqli_query($con,$update_sql);
		}
		else
		{
			mysqli_query($con,"insert into operations (date,starttime,endtime,machine_id,employee_id)values('$date','$starttime','$endtime','$machine_id','$employee_id')");
		}
		header("location:operation.php?id=".$machine_id);
		die();
	}
}
 $sql="select * from operations where machine_id='$id'";
...
<body>
	...
	<div class="form-group row">
		<label for="name" class="col-sm-12 col-form-label"><div class="weight-600 font-30 text-blue">OPERATION</div></label>			
	</div>			
	<div class="form-group row">
	...
	<div class="form-group row">
		<label for="name" class="col-sm-12 col-form-label"><h6>STARTTIME </h6></label>
		<div class="col-sm-12 col-md-10">
		<input type="text" name="starttime" placeholder="Enter STARTTIME" class="form-control time-picker" required value="<?php echo $starttime?>">
		</div>
	</div>
	<div class="form-group row">
		<label for="name" class="col-sm-12 col-form-label"><h6>ENDTIME</h6></label>
		<div class="col-sm-12 col-md-10">
		<input type="text" name="endtime" placeholder="Enter ENDTIME" class="form-control time-picker" required value="<?php echo $endtime?>">
		</div>
	</div>
	<div class="form-group row">
	...
	<div class="form-group row">
		<label for="name" class="col-sm-12 col-form-label"><h6>SELECT  OPERATING EMPLOYEE</h6></label>
		<div class="col-sm-12 col-md-10">
		<select class="form-control" name="employee_id">
										<option>SELECT  OPERATING EMPLOYEE</option>
										<?php
										$res=mysqli_query($con,"select id,name from employees where department_id=5");
										while($row=mysqli_fetch_assoc($res)){
